...
ME_PATH="$PWD"
LOGCAT_PID=0;
REPORT_OUTPUT="HDR_Test_report.txt"
DEF_SLEEP=15;
    ...
    #color_format=("RGB888|0" "YUV444|1" "YUV422|2" "YUV420|3")
    color_format=("YUV444|1" "YUV422|2" "YUV420|3")
...
function test_by_command {
    ...
    do
        ...
        # bit_depth
        for depth in "${bit_depth[@]}"
        do
            curr_bit=`echo $depth | cut -d\| -f1`
            
            # color_format
            for color in "${color_format[@]}"
            do
...
function test_keyinput {
    count=0;
    maxWaitTime=10;
    scroll_index=0;
    curr_res=`test_disp getres | grep "ResID" | cut -d= -f2|xargs`
    #log -d "current ResId=$curr_res"
    # run loop
    while [ $count -lt 16 ]
    do
        log -d "count=$count"
        # go to display menu app
        am start -n com.synaptics.tv.settings/.DisplayActivity
        sleep 2

        # print current resolutiion
        log -i "`test_disp getres | grep ResID`"
        ...
        # 2. scroll by count
        scroll_index=0;
        while [ $scroll_index -lt $count ]
        do
            input keyevent DPAD_DOWN
            scroll_index=`expr $scroll_index + 1`
        done
        ...
        do
            sleep 1
            temp=`test_disp getres | grep ResID`
            temp_res=`echo $temp | cut -d= -f2|xargs`
            log -d "temp = `test_disp getres | grep ResID`"
            if [ "$curr_res" == "$temp_res" ]; then
                waitTime=`expr $waitTime + 1`
            else
                curr_res=$temp_res
                # 4. confirm keep changed resolution after 3 seconds
                input keyevent DPAD_DOWN
                input keyevent DPAD_CENTER
                test_disp getres | grep ResID
                test_disp hdcp state | grep HDCP

                # go to tv app and stay 5 seconds
                # am start -n com.lguplus.android.tv/.pineone.UplusMainActivity
                input keyevent 170
                ...
                result=`getResult`;
                log -d "result = $result"
                echo "\t\tresloution - $temp : $result" >> $REPORT_OUTPUT
                break;
            fi

            if [ $waitTime -eq $maxWaitTime ]; then
            log -d "ignore"
            fi
        done   
        count=`expr $count + 1`
