...
CMD_PREPARE="source workspace/bin/.bashrc_luis && source env.sh"
...
do_execute -i ${SSH_COMMAND} "${CMD_PREPARE} && rm -f ${ANDROID_OUTPUT}/${IMAGE_NAME_PREFIX}*.zip"
if $ONLY_ANDROID; then
    do_execute -i ${SSH_COMMAND} "${CMD_PREPARE} && make installclean && make -j10 && make otapackage"
else
    do_execute -i ${SSH_COMMAND} "${CMD_PREPARE} && ./bcm.ref.sh -d $WORKSPACE_PRODUCT_NAME -z -j 8"
fi
